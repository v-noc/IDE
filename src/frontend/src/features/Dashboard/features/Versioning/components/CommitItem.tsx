...
const CommitItem: React.FC<CommitItemProps> = ({ commit, isLast, isActive, onClick }) => {
    return (
        <div
            onClick={onClick}
            className={`relative flex gap-4 pb-0 group cursor-pointer rounded-xl transition-all duration-200 p-3 mb-2 ${isActive
        ...
        >
            ...
            <div className="flex flex-col items-center">
                <div className={`z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-semibold transition-all duration-200 ${isActive
            ...
            <div className="flex flex-1 flex-col gap-0.5 min-w-0">
                <div className="flex items-center justify-between gap-2">
                    <span className={`text-[13px] font-bold truncate transition-colors ${isActive ? 'text-primary' : 'text-slate-900'
                        }`}>
                        {commit.author}
                    </span>
