...
export function StepCard({ className }: StepCardProps) {
  const [phase, playerSteps, cursor, next, prev, exit] = useWalkthroughStore(
    useShallow((state) => [
      state.phase,
      state.playerSteps,
      state.cursor,
  ...
  if (phase !== "playing" && phase !== "generating" && phase !== "ready") {
    return null;
  }
  ...
  const canNext = cursor < total - 1 || phase === "generating";

  return (
    <div
      className={cn(
        "pointer-events-auto w-full max-w-2xl rounded-xl border border-border bg-background p-4 text-foreground shadow-lg",
        className,
      )}
    >
      ...
      <div className="mb-4 min-h-[3rem] text-sm leading-relaxed text-foreground">
        {waitingForText ? (
          ...
          <p>{step?.text ?? "Press Play to start the tour."}</p>
        )}
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {phase === "ready" && cursor < 0 ? (
            <Button
              type="button"
              size="sm"
              onClick={() => useWalkthroughStore.getState().play()}
            >
              Play
            </Button>
          ) : (
