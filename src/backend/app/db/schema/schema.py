    ...
    def __init__(self, keys: Union[str, list, None] = None):
        if keys is not None:
            if isinstance(keys, str):
                ...
                ValueError(
                    f"keys need to be either str or list but got {keys}")
...
class Schema:
    ...
    def _construct_class(self, class_obj_dict):
        ...
        new_class = type(class_obj_dict["@id"],
                         tuple(superclasses), attributedict)
    ...
    def _construct_object(self, obj_dict):
        ...
        def convert_if_object(obj_type, value):
            ...
            if isinstance(obj_type, str) and obj_type[:4] == "xsd:":
                ...
                if isinstance(value, str):
                    value = [value]
                value = [convert_if_object(obj_type["@class"], x)
                         for x in value]
    ...
    async def commit(
        self, client: AsyncClient, commit_msg: Optional[str] = None, full_replace=False
    ):
        ...
        if self.context["@schema"] is None or self.context["@base"] is None:
            prefixes = await client._get_prefixes()
    ...
    def from_json_schema(
        ...
        def convert_property(prop_name, prop):
            ...
            # it's a subdocument
                ...
                sub_dict = {"@id": prop_name,
                            "@type": "Class", "@subdocument": []}
                for sub_prop_name, sub_prop in prop["properties"].items():
                    sub_dict[sub_prop_name] = convert_property(
                        sub_prop_name, sub_prop)
            ...
            # it's another document
                ...
                if pipe:
                    ...
                    self.from_json_schema(
                        prop_type, defs[prop_type], subdoc=True)
            ...
            # it's enum
                ...
                enum_name = prop_name.replace(
                    "_", " ").capitalize().replace(" ", "")
                enum_dict = {"@id": enum_name,
                             "@type": "Enum", "@value": prop["enum"]}
                if pipe:
                    return enum_dict
                else:
