...
def _finish_response(request_response, get_version=False):
    """Get the response text from a (sync or async) httpx Response.

    For non-streaming responses only.  Streaming responses should be
    handled via ``_finish_streaming_response`` instead.
    ...
    request_response : httpx.Response
        The response object.
    get_version : bool
        If True, also return the ``Terminusdb-Data-Version`` header.

    Returns
    -------
    str or tuple[str, str | None]
        Response text, optionally paired with the data-version header.
...
def _finish_streaming_response(request_response):
    """Return an async line iterator from a streaming httpx response.

    Parameters
    ----------
    request_response : httpx.Response
        A response obtained via ``async with client.stream(...)``.

    Returns
    -------
    AsyncIterator[str]
        Async iterator yielding response lines.
    ...
    """
    if request_response.status_code == 200:
        return request_response.aiter_lines()
    elif 400 <= request_response.status_code < 599:
        raise DatabaseError(request_response)
