...
class ClassRepo(BaseRepo[ClassNode, ClassSchema]):
    ...
    def _merge_update_fields(
        existing_raw: dict, _node: ClassNode, schema: ClassSchema
    ):
        BaseRepo.merge_set_fields(
            schema, existing_raw, CODE_SET_FIELDS_TO_PRESERVE)

    async def create(
        self, class_node: Union[ClassNode, List[ClassNode]], raw: bool = False
    ):
    ...
    async def update_batch(self, classes: List[ClassNode]):
    ...
    async def get_by_id(self, class_id: str, raw: bool = False):
        return await super().get_by_id(class_id, raw)
    ...
    async def get_children(
        self, class_id: str, child_type: list[str]
    ):
