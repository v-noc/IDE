...
class ProjectService():
    ...
    async def create_node(self, project: ProjectNode):
        return await self.project_repos.project_repo.create(project)

    async def create(self, name: str, description: str, path: str):
        return await self.project_repos.project_repo.create(name, description, path)

    async def get(self, project_id: str):
        return await self.project_repos.project_repo.get_by_id(project_id)

    async def get_all(self):
        return await self.project_repos.project_repo.get_all()
