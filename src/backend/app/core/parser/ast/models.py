...
class SubscriptSchema(BaseSchema):
    ...
    elements: List[Union[NameSchema, AttributeSchema]] = Field(
        default_factory=list)  # A flat list of all names found inside
...
class ArgSchema(BaseSchema):
    ...
    name: str
    annotation: Optional[Union[NameSchema,
                               AttributeSchema, SubscriptSchema, str]] = None
...
class FunctionSchema(ParentSchema):
    ...
    return_annotation: Optional[Union[NameSchema,
                                      AttributeSchema, SubscriptSchema, str]] = None
    return_values: List[Union[NameSchema, AttributeSchema,
                              SubscriptSchema, str]] = Field(default_factory=list)
...
class CallSchema(BaseSchema):
    ...
    args: List[Union[NameSchema, AttributeSchema, object]] = Field(
        default_factory=list)  # Only track name/attribute args
...
class AssignSchema(BaseSchema):
    ...
    targets: List[NameSchema | AttributeSchema] = Field(
        default_factory=list)  # Variables being assigned
    value: List[NameSchema | AttributeSchema |
                CallSchema] = Field(default_factory=list)
...
class AnnAssignSchema(BaseSchema):
    ...
    value: List[Union[NameSchema, AttributeSchema,
                      CallSchema]] = Field(default_factory=list)
