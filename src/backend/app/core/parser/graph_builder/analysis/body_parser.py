...
class BodyParser:
    ...
    def process_ast(self, file_scope: ScopeModel, nodes: List[BaseNode]):
        """
    ...
    def _traverse(self, nodes: List[BaseNode], current_scope: ScopeModel, prev_call_id: Optional[str] = None):
        ...
        for node in nodes:
            if isinstance(node, (ClassNode, FunctionNode)):
                ...
                if not child_scope:
                    logger.warning(f"Scope not found for node {node.name} ({node.id}). Skipping.")
                    continue
                ...
                if hasattr(node, "children"):
                    self._traverse(node.children, current_scope, current_call_id)
