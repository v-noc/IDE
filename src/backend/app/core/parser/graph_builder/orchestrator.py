...
from app.core.parser.graph_builder.discovery.change_detector import ChangeDetector, ChangeSet
...
class GraphBuilderOrchestrator:
    def __init__(
        ...
        db: Optional[StandardDatabase] = None, # Main ArangoDB connection (optional for now)
    ):
        ...
        self.scope_manager = ScopeManager(project_node.name)
        ...
        self.collector = Collector(self.project_node, self.scope_manager, self.jedi_manager)
    ...
    def resync(self) -> ChangeSet:
        ...
        """
        logger.info(f"Starting resync for project: {self.project_node.name}")
    ...
    def _process_changes(self, change_set: ChangeSet, current_files: dict):
        ...
        for result in collection_results:
            logger.info(f"Analyzing changes for: {result.file_scope.file_path}")
            ...
            logger.info(f"Processing file body: {result.file_scope.qname}")
            
            # Clear all calls for the file scope (and children will be cleared during traversal)
            self.scope_manager.clear_calls(result.file_scope.id)
            
            # Parse the full AST tree
            # BodyParser will traverse and handle child scopes (clearing their calls too)
            self.body_parser.process_ast(result.file_scope, result.file_ast_nodes)
