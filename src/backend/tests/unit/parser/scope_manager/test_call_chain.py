...
@pytest.mark.asyncio
async def test_call_chain_creation(temp_manager):
    """Test creating a call chain."""
    # Create scopes
    func_a = await temp_manager.create_scope(
        name="func_a",
    ...
    func_b = await temp_manager.create_scope(
        name="func_b",
    ...
    func_c = await temp_manager.create_scope(
        name="func_c",
    ...
    call1 = await temp_manager.create_call(
        caller_id=func_a.id,
        callee_id=func_b.id,
        line=3,
        col=4,
    ...
    call2 = await temp_manager.create_call(
        caller_id=func_b.id,
        callee_id=func_c.id,
        line=12,
        col=4,
    ...
    chain = await temp_manager.get_call_chain(call1.id)
    assert len(chain) == 2
...
@pytest.mark.asyncio
async def test_call_chain_roots(temp_manager):
    ...
    root1 = await temp_manager.create_call(
        caller_id=func_a.id,
        callee_id=func_b.id,
        line=3,
        col=4,
    ...
    await temp_manager.create_call(
        caller_id=func_b.id,
        callee_id=func_c.id,
        line=12,
        col=4,
    ...
    root2 = await temp_manager.create_call(
        caller_id=func_c.id,
        callee_id=func_a.id,
        line=22,
        col=4,
    ...
    # Get all roots
    roots = await temp_manager.get_call_chain_roots()
...
@pytest.mark.asyncio
async def test_call_chain_roots_filtered_by_target_scope(temp_manager):
    ...
    roots_to_b = await temp_manager.get_call_chain_roots(target_scope_id=func_b.id)
    root_ids_to_b = {r.id for r in roots_to_b}
    assert root_ids_to_b == {root1.id}

    # Filter by calls targeting func_c
    roots_to_c = await temp_manager.get_call_chain_roots(target_scope_id=func_c.id)
    ...
    roots_to_a = await temp_manager.get_call_chain_roots(target_scope_id=func_a.id)
