...
@pytest.mark.asyncio
async def test_create_scope_with_manager(temp_manager):
    """Test creating scope using manager."""
    scope = await temp_manager.create_scope(
        name="MyClass",
    ...
    retrieved = await temp_manager.get_scope(scope.id)
...
@pytest.mark.asyncio
async def test_hierarchy_management(temp_manager):
    """Test parent-child linking."""
    parent = await temp_manager.create_scope(
        name="MyClass",
        qname="file.py::MyClass",
        scope_type=ScopeType.CLASS,
        file_path="/path/to/file.py",
    ...
    child = await temp_manager.create_scope(
        name="my_method",
    ...
    await temp_manager.link_parent_child(parent.id, child.id)

    # Get children
    children = await temp_manager.get_children(parent.id)
    assert len(children) == 1
    assert children[0].name == "my_method"


@pytest.mark.asyncio
async def test_call_management(temp_manager):
    """Test call site creation and querying."""
    caller = await temp_manager.create_scope(
        name="func_a",
    ...
    callee = await temp_manager.create_scope(
        name="func_b",
    ...
    call_site = await temp_manager.create_call(
        caller_id=caller.id,
        callee_id=callee.id,
        line=3,
        col=4,
    ...
    calls_from = await temp_manager.get_calls_from(caller.id)
    assert len(calls_from) == 1
    assert calls_from[0]["callee"].name == "func_b"

    # Get calls to callee
    calls_to = await temp_manager.get_calls_to(callee.id)
    assert len(calls_to) == 1
    assert calls_to[0]["caller"].name == "func_a"


@pytest.mark.asyncio
async def test_delete_scope(temp_manager):
    ...
    await temp_manager.delete_scope(scope.id)
