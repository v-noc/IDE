...
def test_group_add_child(create_repos):
    ...
    project = project_service.create("Test Project", "Test Project", "test_project")

    f1 = file_service.create("File 1", "file_1", "File 1", "file1.py", "file1.py")
    f2 = file_service.create("File 2", "file_2", "File 2", "file2.py", "file2.py")

    project_service.add_file(project.id, f1.id)
    project_service.add_file(project.id, f2.id)

    # Create group with one default child (f1)
    group = group_service.create("G", "G", project.key, [f1.key])
    ...
    group_service.add_child_to_group(group.id, f2.id)
...
def test_group_remove_child(create_repos):
    ...
    group = group_service.create("G", "G", project.key, [f1.key, f2.key])
    ...
    ok = group_service.remove_child_from_group(group.id, f2.id)
    assert ok is True
...
def test_group_update_information(create_repos):
    ...
    updated = group_service.update_basic_info(
        group.id,
        name="New Name",
    ...
    fetched = group_service.get(group.id)
