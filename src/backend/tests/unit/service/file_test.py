...
async def test_create_file(project_uow):
    file_service = StructureService(project_uow)
    file_node = FileNode(
        id="file",
        name="Test File",
...
async def test_add_function_to_file(project_uow, create_file, create_function):
    file_service = StructureService(project_uow)
    await file_service.add_child(create_file.id, create_function.id, "function")
...
async def test_add_class_to_file(project_uow, create_file, create_class):
    file_service = StructureService(project_uow)
    await file_service.add_child(create_file.id, create_class.id, "class")
