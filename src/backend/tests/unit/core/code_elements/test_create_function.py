...
def test_create_function_with_inputs_and_outputs(create_function):
    function = create_function
    assert function.inputs == []
    assert function.outputs == []

    function.add_input(name="input1", 
                       position=NodePosition(
                           ...
                           end_col_offset=23), 
                           type="int")  
    function.add_output(name="output1", 
                        position=NodePosition(
                            line_no=1, 
                            col_offset=23, 
                            end_line_no=1, 
    ...
    assert function.inputs == [{"name": "input1", "position": NodePosition(line_no=1, col_offset=23, end_line_no=1, end_col_offset=23), "type": "int"}]
    assert function.outputs == [{"name": "output1", "position": NodePosition(line_no=1, col_offset=23, end_line_no=1, end_col_offset=23), "type": "int"}]